...
export function addDependency(b: IBoard, from: number, to: number) {
    b.sprints.forEach(s => s.tickets.filter(t => t.id === from).forEach(t => t.dependencies.push(to)));
...
export function deleteDependency(b: IBoard, from: number, to: number) {
    b.sprints.forEach(s => s.tickets.filter(t => t.id === from).forEach(t => t.dependencies = t.dependencies.filter(d => d !== to)));
    b.unassigned.filter(t => t.id === from).forEach(t => t.dependencies = t.dependencies.filter(d => d === to));
}